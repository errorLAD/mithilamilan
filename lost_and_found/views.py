...
def create_item(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            item.status = 'PENDING'
            item.save()
            messages.success(request, 'आपका हरायल / भेटल वस्तु विज्ञापन समीक्षा के लिए भेज दिया गया है। एडमिन अनुमोदन के बाद यह लाइव होगा।')
...
def item_detail(request, pk):
    ...
    if item.status != 'APPROVED' and not (request.user.is_staff or request.user == item.user):
        messages.warning(request, 'यह विज्ञापन अभी समीक्षा में है।')
    return render(request, 'lost_and_found/item_detail.html', {'item': item})
